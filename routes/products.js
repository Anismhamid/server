...
router.put('/:productId', auth, async (req, res) => {
    try {
        ...
        if (req.payload.slug !== product.seller.slug) {
            return res
                .status(403)
                .send("Access denied. You can't update this product");
        }
    ...
    try {
        ...
        if (product.image && product.image.publicId) {
            try {
                ...
                console.log(`Deleted image: ${product.image.publicId}`);
        ...
        // Send actual error for debugging (but hide in production)
